 ...
 * (must be d > L, but do not use d >> L otherwise P << 1) apart. the probability P that the needle will intersect one
 * of these lines is: P = (2L )/ (pi * d). This could be used to evaluate PI from throws od the needle: if the needle id
 * throwm down N_thr times ans is observed to land on a line N_hit of those times, we can make an estimation fo Pi from:
