...
#pragma region CONSTANTS
    ...
    constexpr PsiParam delta_params = {.mu = 0.1, .sigma = 0.1}; // Number of step for each block
    constexpr double integration_delta = 1.;                     // Movement of metropolis algorithm
    constexpr PsiParam initial_param{.mu = 1., .sigma = 0.5};
    constexpr PsiParam parameter_reduction_rate = {.mu = delta_params.mu / (n_temperature_step + 1),
                                                   .sigma = delta_params.sigma / (n_temperature_step + 1)};
#pragma endregion CONSTANTS

#pragma region SIMULATION

    SimulatedAnnealing simulator(starting_temperature, ending_temperature, n_temperature_step, same_temperature_step,
                                 integration_delta, delta_params, parameter_reduction_rate, initial_param);
...
#pragma region ENERGY_CALCULATION
    ...
    std::cout << "Final Parameters : " << std::setw(10) << final_wave.mu() << std::setw(10) << final_wave.sigma()
              << '\n';
    ...
    auto sampler = [&final_sampler, &final_wave]() {
        ...
        return final_sampler.get_rng_ptr()->Rannyu(-final_wave.sigma(), final_wave.sigma());
    };
    ...
    for (auto &&energy : v_energies)
    {
        ...
        {
            energy_step = final_wave.hamiltonian(final_sampler.generate(0., PDF, sampler));
        }
