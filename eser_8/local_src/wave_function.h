...
public:
    ...
    WaveFunction(const PsiParam &par) : m_param(par), m_inv_sigma_squared(1. / (par.sigma * par.sigma))
    {
    }
    ...
    WaveFunction(const double mu, const double sigma)
        : m_param({.mu = mu, .sigma = sigma}), m_inv_sigma_squared(1. / (sigma * sigma))
    {
    }

    WaveFunction(const WaveFunction &wf) : m_param(wf.param()), m_inv_sigma_squared(wf.get_inv_sigma_squared())
    {
    }

    WaveFunction(WaveFunction &&wf) : m_param(std::move(wf.m_param)), m_inv_sigma_squared(wf.m_inv_sigma_squared)
    {
    }
    ...
    // Getters
    constexpr double mu() const
    {
        return m_param.mu;
    }
    constexpr double sigma() const
    {
        return m_param.sigma;
    }
    constexpr double get_inv_sigma_squared() const
    {
        return m_inv_sigma_squared;
    }
    constexpr PsiParam param() const
    {
        return m_param;
    }

    // Setters
    void mu(const double mu)
    {
        m_param.mu = mu;
    }
