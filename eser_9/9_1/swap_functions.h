...
namespace PBC_swap
{
template <std::size_t SIZE> std::size_t PBC(const std::size_t idx, const std::size_t offset = 0)
{
...
void swap_ranges(_ForwardIterator array_begin, std::size_t start_first_range, std::size_t start_second_range,
                 std::size_t range_lenght, std::size_t offset)
{
    /// @todo Control that there aren't overlaps between ranges

    for (size_t i = 0; i < range_lenght; i++)
    {
        std::iter_swap(array_begin + PBC_swap::PBC<SIZE>(start_first_range, offset),
                       array_begin + PBC_swap::PBC<SIZE>(start_second_range, offset));
        ++start_first_range;
        ++start_second_range;
    }
}
...
void rotate(_ForwardIterator array_begin, std::size_t first, std::size_t middle, std::size_t last, std::size_t offset)
{
    ...
    if (first != middle)
    {
        ...
        while (true)
        {
            std::iter_swap(array_begin + PBC_swap::PBC<SIZE>(first, offset),
                           array_begin + PBC_swap::PBC<SIZE>(i, offset));
            ++first;
...
void reverse(_ForwardIterator array_begin, std::size_t first, std::size_t last, std::size_t offset)
{
    if (first != last)
        for (; first < --last; ++first)
            std::iter_swap(array_begin + PBC_swap::PBC<SIZE>(first, offset),
                           array_begin + PBC_swap::PBC<SIZE>(last, offset));
}
